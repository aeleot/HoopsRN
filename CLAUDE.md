...
`hooprTests/` has 699 real test methods across 46 suites (counted from the `.xcresult`, 2026-09-25), and `firestore-tests/` has 141 covering all seven collections. The test destination pins `OS=26.5`: the iOS 27 runtime installed alongside it has no iPhone 17, so an unpinned `name=iPhone 17` fails to resolve. `hooprUITests` fails to launch on this project (SpringBoard `RequestDenied`), so don't run it. The fixture data in test files is non-scaffolding — it's either real Firestore documents or realistic test doubles.
